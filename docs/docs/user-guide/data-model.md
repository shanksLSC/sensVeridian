...
| `metadata` | JSON | Per-image rollup of model runs and augmentation provenance. See [schema](#imagesmetadata-json-schema) below. Auto-refreshed by `sv ingest`, augmentation runs, and `sv refresh-metadata`. |

#### `images.metadata` JSON schema

The `metadata` column carries everything you need to know about an image — its
augmentation lineage and every model run associated with it — without joining
across multiple tables. It is updated transactionally at the end of each
per-image processing loop, so it always reflects the latest writes.

```json
{
  "image_id": "329acdb6e9850f346b06d04fd73aec48...",
  "path": "/data3/ssharma8/datasets/mod2_class/images/test/img_0001.jpg",
  "width": 1280,
  "height": 720,
  "augmented_flag": false,
  "augmented_distance_m": null,
  "parent_image_id": null,
  "model_runs": [
    {
      "model_id": "amod",
      "version": "8.2.0",
      "weights_path": "/data3/ssharma8/all-models/AutomotiveMultiObjectDetection/amod-cpnx-8.2.0.h5",
      "weights_sha": "8f2a...",
      "run_id": "mod2_test_amod_c05",
      "conf_threshold": 0.5,
      "present": true,
      "n_detections": 3,
      "class_histogram": {"car": 2, "person": 1},
      "detections": [
        {"bbox": [12, 40, 220, 360], "class_id": 0, "confidence": 0.92},
        {"bbox": [400, 50, 720, 410], "class_id": 0, "confidence": 0.88},
        {"bbox": [800, 200, 880, 480], "class_id": 1, "confidence": 0.74}
      ]
    }
  ]
}
...
| Field | Type | Notes |
|-------|------|-------|
| `image_id` / `path` / `width` / `height` | scalar | Mirrors the row's primary fields. |
| `augmented_flag` | bool | True if this image was generated by an augmentation pipeline (i.e., it has a row in `augmentations` as the `augmented_image_id`). |
| `augmented_distance_m` | float \| null | Augmentation `delta_ft` converted to meters. Null for original images. |
| `parent_image_id` | string \| null | Original image's `image_id` for augmented children; null for originals. |
| `model_runs` | list | One entry per `(run_id, model_id)` row in `predictions_summary`, joined with the corresponding `predictions_raw.payload` and the registered model record. Each entry records the live `conf_threshold` of the runner at refresh time. |
...
### Query 10a: Inspect image metadata for one image

```sql
SELECT json_extract_string(metadata, '$.image_id') AS image_id,
       json_extract(metadata, '$.augmented_flag')        AS augmented_flag,
       json_extract(metadata, '$.augmented_distance_m')  AS augmented_distance_m,
       json_extract(metadata, '$.model_runs')            AS model_runs
FROM images
WHERE image_id = '329acdb6e9850f346b06d04fd73aec48...';
```

### Query 10b: Augmented vs. original counts and distance histogram
